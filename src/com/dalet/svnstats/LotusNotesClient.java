...
public class LotusNotesClient implements Closeable {
    ...
    public static final FilenameFilter FEAT_FILENAME_FILTER = new FilenameFilter() {
        @Override
        public boolean accept(File dir, String name) {
            return name.toLowerCase().endsWith(".xml") && name.length() == "#1234 - AB.xml".length();
        }
    };
    public static final FilenameFilter BUGS_FILENAME_FILTER = new FilenameFilter() {
        @Override
        public boolean accept(File dir, String name) {
            return name.toLowerCase().endsWith(".xml") && name.length() == "#12345 - AB.xml".length();
        }
    };
    ...
    public void createNotesSession() throws NotesException {
        ...
        try {
            ...
            this.bugsDb = connectDbByReplicaId(NOTES_SERVER, BUGS_REPLICAID);
            this.featDb = connectDbByReplicaId(NOTES_SERVER, FEATURES_REPLICAID);
    ...
    private List<Element> fileToLogEntries(File file) throws JDOMException, IOException {
        return new SAXBuilder().build(file).getRootElement().getChildren("logentry");
    }

    public List<RevisionInfoEx> fileToRevisions(File file) throws JDOMException, IOException {
        List<RevisionInfoEx> revisions = new ArrayList<>();
        for (Element logentry : fileToLogEntries(file)) {
            revisions.add(new RevisionInfoEx(logentry));
        }
        return revisions;
    }
    ...
    public String buildFeatureHtmlLink(Document doc) throws NotesException {
